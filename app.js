...
function bindStaticEvents() {
  ...
  document
    .querySelectorAll("[data-close-compare]")
    .forEach(button => {
  ...
  els.compareA?.addEventListener(
    "change",
    renderCompare
  );

  els.compareB?.addEventListener(
    "change",
    renderCompare
  );

  document
    .querySelector("#fullscreenReader")
    ?.addEventListener("click", () => {
...
function renderCompare() {
  ...
  const a =
    state.books.find(
      book => book.id === els.compareA.value
    ) || state.books[0];

  const b =
    state.books.find(
      book => book.id === els.compareB.value
    ) || state.books[1];
